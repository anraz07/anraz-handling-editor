...
export const TuningSlider: React.FC<TuningSliderProps> = ({ attrName, meta, value, onChange }) => {
  ...
  return (
    <div className="flex flex-col mb-3 bg-white/5 p-2 border-l-4 border-transparent hover:border-gta-accent transition-colors">
      <div className="flex justify-between items-center mb-1">
        <label className="text-sm text-gta-text font-bold tracking-wide uppercase" title={meta.desc}>
          {attrName}
        </label>
        <input 
          type="text" 
      ...
      <div className="text-[10px] text-white/50 mt-1 uppercase">{meta.desc}</div>
    </div>
  );
};
